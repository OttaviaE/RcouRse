...
>conditional probability of obtaining a test stastic that is at least as extreme as the one observed, given that the null hyphothesis is
...
$$\chi^2 = \displaystyle \sum_{i=1}^{n}\frac{(x_{ij} - \hat{x}_{ij})^2}{\hat{x}_{ij}}, \, \, \chi^2 \sim \chi^2(r-1)$$
...
<!-- \begin{tabular}{l l} -->
<!-- \multicolumn{2}{l}{Formulae used in \texttt{lm()} and \texttt{glm()}.}\\ -->
<!-- \texttt{y ~ 1 + x} & $y_i = \beta_0 + \beta_1 x_i + \varepsilon_i$ \\ -->
<!-- \texttt{y ~ x} & (same but short)\\ -->
<!-- \texttt{y ~ 0 + x} & $y_i = \beta_1 x_i + \varepsilon_i$ \\ -->
<!-- \texttt{y ~ x_A * x_B} & $y_i = \beta_0 + \beta_1 x_i + \beta_2 x_j + (\beta_1 \beta_2)x_{ij}  + \varepsilon_{ij}$ \\ -->
<!-- \end{tabular} -->
