...
`data` is a data frame where the original numerical varibales are converted in numeric vectors and character variables are converted in factors. 
...
```{r eval=FALSE}
write.table(data, # what you eant to write 
...
grid.arrange(murder_raw, murder_lm, 
             nrow=1) # plots forced to be on same raw
```
