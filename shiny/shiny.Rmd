...
The appearance of the app didn't change at all, but now we don't have to type the selection of the code for every time we need to use the data frame: It's now stored in the shelf and we can call for it every time we want by using the `$` sign...Just like in R!

Still, there's something I don't like: Everytime we select a different data frame, the output change immediately. An action button would be useful.
